...
if uploaded_file is not None:
    ...
    # color_rgb1 = np.uint8([[[255, 170, 0]]])
    # color_hsv = cv2.cvtColor(color_rgb1, cv2.COLOR_RGB2HSV)[0][0]

    # # Define the range of the specific color in HSV
    # lower_bound_orange = np.array([color_hsv[0] - 0, 50, 50])
    # upper_bound_orange = np.array([color_hsv[0] + 0, 255, 255])
    ...
    st.image(cv2.cvtColor(result_with_bbox, cv2.COLOR_BGR2RGB), caption='Overall Result with Bounding Box', use_column_width=True)
